...
parser.add_argument(
    "--save_dir",
    type=str,
    default=f"{MY_HOME}/results/saved_csv",
    help="Directory to save the CSV file.",
)
args = parser.parse_args()

# Correctly set the tracking URI
mlflow.set_tracking_uri(f"file://{MY_HOME}/logs/mlflow/mlruns")
...
def extract_dataframe(
    ...
    if override_saved is False:
        for file in os.listdir(f"{MY_HOME}/results/saved_csv"):
            if file == "{}_id_{}.csv".format(experiment_name, exp_id):
                print("The experiment is already saved")
                return pd.read_csv(
                    f"{MY_HOME}/results/saved_csv/{experiment_name}_id_{exp_id}.csv"
                )
