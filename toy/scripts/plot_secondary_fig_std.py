...
def gen_plot(
    dataset, num_hypothesis, ax, plot_unweighted, base_dir, list_folder_names, plot_std
):
    ...
    Y_LIM_MIN = {
        "9": {
            "single-gaussian-not-centered": -0.5,
            "rotating-two-moons": 0.7,
            "changing-damier": 1.2,
            "mixtures-uni-to-gaussians": 0.8,
        },
    }

    Y_LIM_MAX = {
        "9": {
            "single-gaussian-not-centered": 2.0,
            "rotating-two-moons": 2.5,
            "changing-damier": 3.0,
            "mixtures-uni-to-gaussians": 4.0,
        },
        "16": {
            "single-gaussian-not-centered": 2.0,
            "rotating-two-moons": 2.5,
    ...
    if num_hypothesis == 9:
        lim_min = Y_LIM_MIN["9"][dataset]
    else:
        lim_min = ax.get_ylim()[0]

    lim_max = Y_LIM_MAX[str(num_hypothesis)][dataset]

    if dataset == "single-gaussian-not-centered":
        # ax.set_title(f'$\\it{{Single\;Gaussian}}$', fontsize=20)
        ax.set_title("Single Gaussian", fontsize=25)
        # a = ax.get_ylim()[0]
        ax.set_ylim(lim_min, lim_max)  # single gaussian
        ax.set_ylabel("NLL", fontsize=20)
    elif dataset == "mixtures-uni-to-gaussians":
        # ax.set_title(f'$\\it{{Uniform\;to\;Gaussians}}$', fontsize=20)
        ax.set_title("Uniform to Gaussians", fontsize=25)
        ax.set_ylim(lim_min, lim_max)  # mixture uni to gaussians
        ax.set_ylabel("")
    elif dataset == "changing-damier":
        # ax.set_title(f'$\\it{{Changing\;Damier}}$', fontsize=20)
        ax.set_title("Changing Damier", fontsize=25)
        ax.set_ylim(lim_min, lim_max)  # changing damier
        ax.set_ylabel("NLL", fontsize=20)
    elif dataset == "rotating-two-moons":
        # ax.set_title(f'$\\it{{Rotating\;Two\;Moons}}$', fontsize=20,pad=10)
        ax.set_title("Rotating Two Moons", fontsize=25)
        ax.set_ylim(lim_min, lim_max)  # rotating moons
        ax.set_ylabel("")
...
def make_subplots(
    ...
    fig.legend(
        ...
        ncol=5,
        loc="upper center",
        bbox_to_anchor=(0.5, 0.94),  # Adjust this tuple to shift the legend
        #  prop={'size': 12},
        fontsize=15,
    )
    fig.suptitle("With {} hypotheses".format(num_hypotheses), fontsize=30, y=0.99)
    # fig.suptitle('', fontsize=1,y=1.00)
...
if __name__ == "__main__":
    ...
    # parser.add_argument("--list_folder_names", type=list, default=['saved_csv','saved_csv_seed1244','saved_csv_seed1245'])
    parser.add_argument(
        "--list_folder_names", type=str, default="${MY_HOME}/results/considered_csv"
    )
    # parser.add_argument("--list_folder_names", type=list, default=['saved_csv_seed1244','saved_csv_seed1245'])
    parser.add_argument("--plot_std", type=str, default="True")
    parser.add_argument("--num_hypotheses", type=int, default=9)
    parser.add_argument("--plot_unweighted", type=str, default="False")
