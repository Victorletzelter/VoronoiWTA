...
def build_paths(base_dir, dataset):
...
def construct_nll_dict(base_dir, dataset="changing-damier", num_hypotheses=20):
    csv_file = build_paths(base_dir=base_dir, dataset=dataset)[0]
...
def gen_plot(dataset, num_hypothesis, save_path, base_dir, plot_title=False):
    ...
    models = construct_nll_dict(
        base_dir=base_dir, dataset=dataset, num_hypotheses=num_hypothesis
    )
